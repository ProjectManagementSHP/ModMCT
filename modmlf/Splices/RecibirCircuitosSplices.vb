...
Public Class RecibirCircuitosSplices
    ...
    '                Dim query As String = $"UPDATE tblAtados SET CellSpliceReceived = 1, CellSplice='{lblCelda.Text}' WHERE idAtado = @idAtado"
    '                Using command As New SqlCommand(query, connection)
    ...
    Private Sub OK_Click(sender As Object, e As EventArgs) Handles OK.Click
        ...
        txtIdAtado.Text = ""
        LimpiarTextBox()
        MsgBox("Se recibio Splice satisfactoriamente")

    End Sub
    Private Sub ObtenerInfoAtado(id As String)
        Dim query As String = "SELECT idAtado,B.TermA, B.TermB, A.IDCWO, Qty, A.Route, B.CWO, B.WIP, B.JoinA, B.JoinB, B.Wire, B.WID FROM tblatados A inner join tblCWOSerialNumbers B on A.IDCWO=B.IDCWO  WHERE idatado = @idatado and CellSpliceReceived = 0 and (joinA like '%SP%' or joinB like '%SP%' ) "
