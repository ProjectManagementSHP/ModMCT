...
Public Class HoraXHora
    ...
    Public Event CellPainting As System.Windows.Forms.DataGridViewCellPaintingEventHandler ' Evento para desplegar los split de los encabezados de la Grid DataGridView1 
    ...
    Private Sub Hrs(Minutos As Integer)
        Try
            ...
            While Maquinas <= 11
                If Maquinas = 0 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 0 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 1 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 1 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 2 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 2 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 3 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 3 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 4 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 4 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 5 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 5 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 6 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 6 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 7 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 10 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 8 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 11 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 9 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 12 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 10 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 13 and det.Balance > 0 order by c.Id asc,IdSort asc"
                If Maquinas = 11 Then Consulta = "select det.CWO,WireBalance,Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado' from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq = 14 and det.Balance > 0 order by c.Id asc,IdSort asc"
    ...
    Private Sub btnDet2_Click(sender As Object, e As EventArgs) Handles btnDet2.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=2 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 2"
    End Sub
    Private Sub btnDet5_Click(sender As Object, e As EventArgs) Handles btnDet5.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=5 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 5"
    End Sub
    Private Sub btnDet10_Click(sender As Object, e As EventArgs) Handles btnDet10.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=10 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 10"
    End Sub
    Private Sub btnDet12_Click(sender As Object, e As EventArgs) Handles btnDet12.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=12 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 12"
    End Sub
    Private Sub btnDet13_Click(sender As Object, e As EventArgs) Handles btnDet13.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=13 order by Id asc, IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 13"
    End Sub
    Private Sub btnDet14_Click(sender As Object, e As EventArgs) Handles btnDet14.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',0 [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=14 order by Id asc,IdSort asc")
    ...
    Private Sub btnDet3_Click(sender As Object, e As EventArgs) Handles btnDet3.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',convert(float,round(TSetup / 60 + TRuntime / 60,2,1)) [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=3 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 3"
    End Sub
    Private Sub btnDet4_Click(sender As Object, e As EventArgs) Handles btnDet4.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',convert(float,round(TSetup / 60 + TRuntime / 60,2,1)) [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=4 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 4"
    End Sub
    Private Sub btnDet6_Click(sender As Object, e As EventArgs) Handles btnDet6.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',convert(float,round(TSetup / 60 + TRuntime / 60,2,1)) [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=6 order by Id asc,IdSort asc")
        lblmaquina.Text = "Maquina seleccionada: 6"
    End Sub
    Private Sub btnDet11_Click(sender As Object, e As EventArgs) Handles btnDet11.Click
        llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',convert(float,round(TSetup / 60 + TRuntime / 60,2,1)) [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.Maq=11 order by Id asc,IdSort asc")
    ...
    Public Sub filtrarDatos(ByVal buscar As String)
        Try
            ...
            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 2) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da2 = New SqlDataAdapter(query, cnn)
            da2.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 10) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da3 = New SqlDataAdapter(query, cnn)
            da3.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 13) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da4 = New SqlDataAdapter(query, cnn)
            da4.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 14) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da5 = New SqlDataAdapter(query, cnn)
            da5.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 0) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da6 = New SqlDataAdapter(query, cnn)
            da6.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 5) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da7 = New SqlDataAdapter(query, cnn)
            da7.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 12) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da8 = New SqlDataAdapter(query, cnn)
            da8.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 3) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da11 = New SqlDataAdapter(query, cnn)
            da11.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 4) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da12 = New SqlDataAdapter(query, cnn)
            da12.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 6) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da13 = New SqlDataAdapter(query, cnn)
            da13.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select PN, CONVERT(int,SUM(balance)) [Qty],CONVERT(int,SUM(ISNULL(TagAsignado,0))) [Tag Asignado],Hold from tblBOMCWO where CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq = 11) and CONVERT(int,Balance) > 0 and PN not like 'S%' and PN like @filtro group by PN,Hold"
            da14 = New SqlDataAdapter(query, cnn)
            da14.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))
            '------------------------
            query = "select TAG,bw.PN,'' as [Ultima Maquina usada],OutDate [Fecha de Salida],ml.Balance From tblBOMCWO As bw inner Join tblItemsTags As ml On bw.PN = ml.PN Where bw.CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq in (select Maq from tblMaqRates where Active = 1)) and bw.Balance > 0 and bw.PN not like 'S%' and bw.PN like @filtro and Status='NoAvailable' and ml.Balance > 0 and bw.Balance > 0 group by TAG, bw.PN,OutDate,ml.Balance"
            da9 = New SqlDataAdapter(query, cnn)
            da9.SelectCommand.Parameters.AddWithValue("@filtro", String.Format("%{0}%", buscar))

            query = "select TAG,bw.PN,CONVERT(int,ml.Balance) [Balance],Location From tblBOMCWO As bw inner Join tblItemsTags As ml On bw.PN = ml.PN Where bw.CWO in (select CWO from tblCWO where WSort in (20,25,29) and Status='OPEN' and (Id is not null or Id > 0) and Maq in (select Maq from tblMaqRates where Active = 1)) and bw.PN not like 'S%' and bw.PN like @filtro and Status='Available' and ml.Balance > 0 and bw.Balance > 0 and (ml.Balance >= bw.Balance) and (bw.TagAsignado is null or bw.TagAsignado = 0) group by TAG, bw.PN,OutDate,ml.Balance,ml.Location"
    ...
    Private Sub DataGridView1_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DataGridView1.CellDoubleClick
        If lblHrsConsultadas.Visible = False Then
            ...
            If cabecera = "CWO" Then
                llenaGridWipCutCard("select WIP,det.CWO,AU,Rev,WID,IdSort,LengthWire [Length],Balance,WireBalance,isnull(convert(int,(LengthWire * Balance) * 1.0 / 304.8),0) [Total Ft],Wire,TermA,TermB,ISNULL(TSetup + TRuntime,0) [Total Minutos],null as 'Acumulado',convert(float,round(TSetup / 60 + TRuntime / 60,2,1)) [Total Hrs] from tblWipDet det inner join tblCWO c on c.CWO=det.CWO where c.WSort in (20,25,29) and c.Status='OPEN' and (c.Id is not null or c.Id > 0) and c.CWO= '" + Me.DataGridView1.CurrentCell.Value.ToString() + "' and c.Maq in (select Maq from tblMaqRates where Active = 1) order by Id asc,IdSort asc")
            End If
        Else
